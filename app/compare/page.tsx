...
function RoundTripCell({ firm }: { firm: PropFirm }) {
        ...
        {sortedRows.map(({ platform, mnqNq }) => {
          ...
          return (
            <div
              ...
              title={`${label} ${mnqNq}`}
            >
              <span className="inline-flex h-[11px] w-[11px] shrink-0 items-center justify-center rounded border border-[color:var(--cmp-sage-border)] bg-black/35 p-px shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                {src ? (
                  <Image
                    src={src}
                    alt=""
              ...
              <span className="text-[11px] tabular-nums leading-none text-[color:var(--cmp-steel)]">
                {mnqNq}
              </span>
            </div>
          );
  ...
  return (
    <div className="whitespace-pre-line text-[12px] leading-snug text-[color:var(--cmp-steel)]">
      {firm.roundTripMnqNq}
    </div>
  );
}
...
function scoreRingConfig(score: number) {
  ...
  if (score >= 8) {
    ...
    return {
      ...
      trackColor: "rgba(28, 42, 39, 0.72)",
      innerClass: "bg-[#0d1513] text-[color:var(--cmp-mint)]",
    } as const;
  }
  if (score >= 7) {
    ...
    return {
      ...
      trackColor: "rgba(28, 42, 39, 0.72)",
      innerClass: "bg-[#0d1513] text-amber-300/95",
    } as const;
  }
  if (score >= 4) {
    // Low-mid → orange ring
    return {
      ...
      trackColor: "rgba(28, 42, 39, 0.72)",
      innerClass: "bg-[#0d1513] text-orange-300/95",
    } as const;
  }
  // Very low → red ring
  return {
    ...
    trackColor: "rgba(28, 42, 39, 0.72)",
    innerClass: "bg-[#0d1513] text-red-300/95",
  } as const;
}
...
function filterPill(active: boolean) {
  return active
    ...
    : "border-[color:var(--cmp-sage-border)] bg-[color:var(--cmp-sage-softer)] text-[color:var(--cmp-steel)] hover:border-[color:var(--cmp-sage-strong)] hover:bg-[color:var(--cmp-sage-soft)] hover:text-[color:var(--foreground)]";
}
...
function ActiveFilterChip({
  ...
  return (
    <span className="inline-flex shrink-0 items-center gap-0.5 rounded-full border border-[color:var(--cmp-sage-border)] bg-[color:var(--cmp-sage-soft)] py-0.5 pl-2 pr-0.5 text-[10px] font-medium leading-tight tracking-normal text-[color:var(--foreground)]">
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex shrink-0 items-center gap-1.5 rounded-xl border px-3.5 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] transition ${
                        active
                          ? "border-[color:var(--cmp-sage-strong)] bg-[color:var(--cmp-sage-soft)] text-[color:var(--foreground)] shadow-sm shadow-black/15"
                          : "border-[color:var(--cmp-sage-border)] bg-[color:var(--cmp-sage-softer)] text-[color:var(--cmp-steel)] hover:border-[color:var(--cmp-sage-strong)] hover:bg-[color:var(--cmp-sage-soft)] hover:text-[color:var(--foreground)]"
                      }`}
                    >
                      {label}
                  ...
                  */}
                  ...
                  {filteredFirms.map((firm, rowIndex) => {
                    ...
                    <div
                      ...
                      id={`firm-row-${firm.id}`}
                      className={`${COMPARE_TABLE_ROW_BASE} ${
                        rowIndex % 2 === 0
                          ? "bg-[color:var(--cmp-row-a)]"
                          : "bg-[color:var(--cmp-row-b)]"
                      ...
                      >
                        {(() => {
                          ...
                          <button
                            ...
                            data-row-click-ignore="true"
                            onClick={() => {
                              void copyPromoToClipboard(promo);
                            }}
                            className="rounded-lg border border-[color:var(--cmp-sage-border)] bg-[color:var(--cmp-sage-softer)] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-[color:var(--foreground)] transition hover:border-[color:var(--cmp-sage-strong)] hover:bg-[color:var(--cmp-sage-soft)]"
                          >
                            {promo}
                          </button>
                          ) : (
                      ...
                      <div className="flex flex-col items-center justify-center text-center text-[13px] tabular-nums leading-tight">
                        {isActivationFree(firm) ? (
                          <>
                            ...
                            {firm.activationNote ? (
                              <p className="mt-0.5 text-[11px] text-[color:var(--cmp-mint-dim)]">
                                {firm.activationNote}
                              </p>
                            ) : null}
                          </>
